    ...
    def __sell_call_stoploss(self, scan_entry, scan_data, watchlist, portfolio, client):
        ...
        if scan_entry.stop_loss < scan_entry.current_price:
            ...
            if not scan_entry.order_id:
                order_id = client.order_option_buy_close_limit('debit', scan_entry.current_price,
                    watchlist.ticker, portfolio.units/100, watchlist.option_expiry, watchlist.option_strike, 'call')

                order_status = client.get_open_option_order_status(order_id)
                
                # Order
                order = OpenOrder(watchlist_id_list=[watchlist.id],
                            transaction_type_list=[TransactionType.BUY.value],
                            created_datetime=open_order['created_at'],
                            price=scan_entry.current_price,
                            units=portfolio.units,
                            brine_id=order_id,
                            closing_strategy=order_status['closing_strategy'],
                            opening_strategy=order_status['opening_strategy'],
                            source=PortFolioSource.BRINE.value)
                order.save()
            
                # Order closing triggered. Disalbe active track
                scan_entry.order_id = order.id
                ...
                if order_status['state'] == 'cancelled':
                    ...
                    logger.error('__sell_call_stoploss: Order state {} is unknown. Ignoring'
                        .format(order_status))
                    return
