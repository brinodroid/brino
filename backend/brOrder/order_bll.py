...
def get_open_orders(watchlists_for_ticker):
    open_orders_list = OpenOrder.objects.filter()
    watchlist_id_list = [watchlist.id for watchlist in watchlists_for_ticker]

    res_open_orders_list = []
    for open_order in open_orders_list:
        watchlist_id_list_in_order = open_order.watchlist_id_list.split(',')
        for watchlist_id in watchlist_id_list_in_order:
            if int(watchlist_id) in watchlist_id_list:
                res_open_orders_list.append(open_order)
    return res_open_orders_list
...
def submit_market_order_to_client(watchlist, transaction_type, units, action):
    client = get_client()

    logger.info('submit_market_order_to_client: watchlist {},  transaction_type {}, units {}, action {}'
                .format(watchlist, transaction_type, units, action))
    if watchlist_bll.is_option(watchlist):
        latest_price, ask_price, bid_price = watchlist_bll.get_watchlist_latest_price(
            watchlist)

        if transaction_type == TransactionType.SELL.value:
            # For selling at market, go with the bid_price
            price = bid_price
        else:
            # For buying at market, go with the ask_price
            price = ask_price
    ...
    # Its a stock
    submitted_order = _submit_stock_market_order_to_client(
        watchlist, transaction_type, units, client)
...
def _update_open_stock_orders_legs(watchlist, submitted_order, client):
    ...
    instrument_data = client.get_stock_instrument_data(
        submitted_order['instrument_url'])
...
def _save_submitted_stock_order(watchlist, submitted_order, client):
    watchlist_id_list, transaction_type_list = _update_open_stock_orders_legs(
        watchlist, submitted_order, client)
...
def _submit_limit_order_to_client(watchlist, transaction_type, units, price, action):
    ...
    # Its a stock
    submitted_order = _submit_stock_limit_order_to_client(
        watchlist, transaction_type, units, price, client)
...
def _save_submitted_option_order(watchlist, submitted_order, client):
    watchlist_id_list, transaction_type_list = _update_open_option_orders_legs(
        watchlist, submitted_order, client)
    ...
    open_order = OpenOrder(watchlist_id_list=watchlist_id_list_text,
                           ...
                           price=submitted_order['brino_entry_price'],
                           units=float(
                               submitted_order['quantity'])*utils.option_unit_multiplier,
                           brine_id=submitted_order['id'],
